最近、実験データを機械学習させろとか言われることが増えました。（管理職一同が機械学習を分かっていないのは置いておいて）実験データを機械学習のツール（pandasとか）に読み込ませようとしたときに一番問題なのが、実験条件がデータのファイルにラベリングされていないということです。根性で全ファイル名に書き込むのは、明らかに大変すぎます。

これを解決するために作成したのが、EDTA(Expelimental Data Tag Authiring)です。
小生は本職は実験と設計とシミュレーションですので、拙い点とバグはご容赦ください。データはバックアップを取ってから使ってください。
...
機能は以下のとおりです。
＊（重要）当たり前ですが、実験結果に影響を与えていると考えられる条件がラベルに含まれていなければ、最後に実験条件付きのデータまとめ*.csvが生成されても、そこから何の解析結果も得られないと思います。

なお、ラベル付けするデータの一番上の階層を"root path"と呼ぶことにします。このroot直下は、ファイルに対しては2.のサブプロセスは実行されません。
